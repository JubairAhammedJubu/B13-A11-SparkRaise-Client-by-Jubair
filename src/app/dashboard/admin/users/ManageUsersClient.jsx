...
import {
    Users, Search, Shield, Rocket, Heart,
    Loader2, CheckCircle, AlertTriangle,
    Mail, Calendar, Coins, Trash2
} from 'lucide-react';
...
export default function ManageUsersClient({ currentUser }) {
    ...
    const handleDelete = async (userId, userName) => {
        ...
        if (!confirm(`Remove ${userName} from the platform permanently?`)) return;
        setActionLoading(userId + 'delete');
        try {
            const result = await deleteUser(userId);
            if (result?.deletedCount || result?.acknowledged) {
                toast.success(`${userName} removed`);
                setUsers(prev => prev.filter(u => (u.id !== userId && u._id?.toString() !== userId)));
            } else {
    ...
    const counts = {
        all: users.length,
        supporter: users.filter(u => u.role === 'supporter').length,
        ...
        <div className="p-6 pt-8 max-w-7xl mx-auto space-y-6 mt-4">
            ...
            >
                {[
                    ...
                    { label: 'Supporters', value: counts.supporter, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-900/20', icon: Heart },
                    { label: 'Creators', value: counts.creator, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-900/20', icon: Rocket },
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                                {filtered.map((user, i) => {
                                    const roleConfig = ROLE_CONFIG[user.role] || ROLE_CONFIG.supporter;
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-4">
                                                <span className="flex items-center gap-1.5 text-sm font-semibold text-gray-700 dark:text-gray-300">
                                                    ...
                                                    {user.role === 'creator' ? (user.raised_credits || 0) : (user.credits || 0)}
                                                </span>
                                            </td>
                                                ...
                                                <div className="flex items-center justify-end gap-1.5 flex-wrap">
                                                    {isSelf ? (
                                                        ...
                                                        <>
                                                            {user.role !== 'supporter' && (
                                                                <button
                                                                    onClick={() => handleRoleChange(userId, 'supporter', user.name)}
                                                                ...
                                                                >
                                                                    {actionLoading === userId + 'supporter' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Heart className="w-3 h-3" />}
                                                                    Supporter
                                                                </button>
                                                            )}
                                                            ...
                                                            {user.role !== 'creator' && (
                                                                <button
                                                                    onClick={() => handleRoleChange(userId, 'creator', user.name)}
                                                                    disabled={!!actionLoading}
                                                                ...
                                                                >
                                                                    {actionLoading === userId + 'creator' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Rocket className="w-3 h-3" />}
                                                                    Creator
                                                                </button>
                                                            )}
                                                            ...
                                                            <button
                                                                ...
                                                                disabled={!!actionLoading}
                                                                className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 text-xs font-semibold hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-all disabled:opacity-50"
                                                            >
                                                                {actionLoading === userId + 'delete' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-3 h-3" />}
                                                                Remove
                                                            </button>
                                                        </>
                                                    )}
