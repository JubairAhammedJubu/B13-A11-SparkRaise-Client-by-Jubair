...
export default function CreatorProfile({ user }) {
    ...
    const modalPreviewSrc = pendingImage || savedImage ||
        `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=10b981&color=fff&size=96`;
    ...
    return (
        <div className="p-6 pt-8 max-w-4xl mx-auto space-y-6 mt-16">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Creator Profile</h1>
                <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
                    Manage your creator information and photo
                </p>
            </motion.div>
                ...
                <div className="px-6 pb-6">
                    <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 -mt-12 mb-5">
                        <div className="relative inline-block">
                            ...
                            <button
                                onClick={() => setIsEditingImage(true)}
                                className="absolute -bottom-2 -right-2 w-8 h-8 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 flex items-center justify-center shadow-md hover:scale-110 transition-transform"
                            >
                        ...
                        <div className="flex items-center gap-2 pb-1">
                            ...
                            <span className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800">
                                🚀 Creator
                            </span>
                        </div>
                    </div>
                            ...
                            {isEditingName ? (
                                <div className="flex items-center gap-2 flex-1">
                                    <input
                                        ...
                                        className="flex-1 px-3 py-1.5 text-lg font-bold bg-gray-50 dark:bg-gray-800 border border-emerald-400 rounded-xl text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
                                        autoFocus
                                    />
                                    <button
                                        onClick={handleSaveName}
                                        disabled={isLoading}
                                        className="p-1.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-500 transition-colors disabled:opacity-50"
                                    >
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center mb-5 gap-2">
                            <img
                                src={modalPreviewSrc}
                                alt="Preview"
                                className="w-24 h-24 rounded-2xl object-cover border-4 border-emerald-100 dark:border-emerald-900 shadow-md"
                            />
                            {pendingImage && (
                        ...
                        {imageTab === 'upload' ? (
                            <div
                                ...
                                className="border-2 border-dashed border-gray-200 dark:border-gray-700 hover:border-emerald-400 dark:hover:border-emerald-600 rounded-xl p-6 text-center cursor-pointer transition-all group"
                            >
                                <input
                                    ref={fileInputRef}
                                    type="file"
                                ...
                                {isUploadLoading ? (
                                    ...
                                    <Upload className="w-8 h-8 text-gray-400 group-hover:text-emerald-500 mx-auto mb-2 transition-colors" />
                                )}
                        ...
                        <div className="flex gap-2 mt-4">
                            ...
                            <button
                                ...
                                className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 text-white font-semibold text-sm shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
                            >
                                {isLoading ? (
            ...
            >
                {[
                    { icon: User, label: 'Full Name', value: name, color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },
                    { icon: Mail, label: 'Email Address', value: user?.email, color: 'text-teal-500', bg: 'bg-teal-50 dark:bg-teal-900/20' },
                    { icon: Rocket, label: 'Account Role', value: '🚀 Creator', color: 'text-cyan-500', bg: 'bg-cyan-50 dark:bg-cyan-900/20' },
            ...
            >
                ...
                <div className="flex flex-wrap gap-3">
                    <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800">
                        ...
                        <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400">Active Creator</span>
                    </div>
                    {user?.emailVerified && (
