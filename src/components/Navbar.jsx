...
import { Sun, Moon, ChevronDown, LayoutDashboard, LogOut, User, Shield, Rocket, Heart, Coins } from "lucide-react";
import { FaGithub } from "react-icons/fa";

// Doc requirement: "Join as Developer" button, visible for both logged-in and
// logged-out users, redirects to the client-side GitHub repository.
const CLIENT_REPO_URL = "https://github.com/JubairAhammedJubu";
import { useSession, signOut, clearAccessToken } from '@/lib/auth-client';
...
const Navbar = () => {
  ...
  // Live credit balance — session.user.credits is cached and goes stale
  // as soon as the Express server updates the DB directly (see useLiveCredits).
  const { credits: liveCredits } = useLiveCredits(user);
  ...
  const navLinks = [
    ...
    ...(user ? [{ name: "Dashboard", href: dashboardLinks[user?.role || 'supporter'] }] : []),
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center justify-center flex-1 mx-8">
              <div
                className={`flex items-center space-x-1 rounded-full p-1 transition-all duration-500 ${
                      ...
                      className={`px-4 py-2 text-sm font-medium rounded-full transition-all duration-300 ${
                        isActive
                          ...
                          : isTransparent && isDark
                            ...
                            : isTransparent && !isDark
                              ? "text-white drop-shadow-sm hover:text-white hover:bg-black/40"
                              : "text-gray-800 dark:text-gray-200 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-gray-200 dark:hover:bg-gray-700"
                      }`}
                    >
                      {link.name}
                    </Link>
              ...
              {user?.role === "supporter" && (
                <Link
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-semibold transition-all duration-200 ${
                    isTransparent
                      ? "border-white/20 bg-white/10 text-white hover:bg-white/20"
                      : "border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 hover:bg-amber-100"
                  }`}
                >
              ...
              <div className="px-4 py-5 space-y-2">
                ...
                {user?.role === "supporter" && (
                  <Link
                    ...
                    className="flex items-center gap-2 px-4 py-3 mb-1 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 text-amber-600 dark:text-amber-400 text-sm font-semibold"
                  >
                    <Coins className="w-4 h-4" />
                    {liveCredits.toLocaleString()} credits available
                  </Link>
                )}
                ...
                <Link
                  ...
                  rel="noopener noreferrer"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="flex items-center gap-2 px-4 py-3 text-sm font-medium rounded-xl text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-indigo-600 dark:hover:text-indigo-400"
                >
